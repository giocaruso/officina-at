...
// ==========================================
// --- GESTIONE BANNER COOKIE E ANALYTICS ---
// ==========================================

function gestisciCookie(scelta) {
    // 1. Salva la scelta nel localStorage del browser
    localStorage.setItem('consenso_cookie', scelta);
    
    // 2. Nasconde visivamente il banner
    const banner = document.getElementById('cookie-banner');
    if (banner) banner.style.display = 'none';
    
    // 3. Comunica l'aggiornamento a Google Analytics se l'utente accetta
    if (scelta === 'accettati') {
        if (typeof gtag === 'function') {
            gtag('consent', 'update', {
            ...
            console.log("Cookie accettati: tracciamento Analytics attivato.");
        }
    } else {
        console.log("Cookie rifiutati: tracciamento Analytics rimane bloccato.");
    }
}

function apriPreferenzeCookie() {
    // Crea l'overlay scuro di sfondo per il pannello (Modal)
    ...
    // Inietta l'interfaccia HTML nel box
    modalBox.innerHTML = `
        <h3 style="margin-top: 0; color: var(--secondary, #333); font-size: 1.5rem;">Personalizza i Cookie</h3>
        <p style="color: #555; font-size: 0.95rem; line-height: 1.5; margin-bottom: 20px;">
            Scegli quali categorie di cookie autorizzare. I cookie tecnici sono necessari per il funzionamento del sito.
        </p>
        
        <div style="margin-bottom: 15px; padding: 12px; border: 1px solid #eee; border-radius: 8px; background: #f9f9f9;">
            <label style="display: flex; align-items: center; justify-content: space-between; font-weight: bold; color: #333; cursor: not-allowed;">
                <span>🛡️ Cookie Tecnici (Obbligatori)</span>
        ...
        <div style="margin-bottom: 15px; padding: 12px; border: 1px solid #eee; border-radius: 8px;">
            <label style="display: flex; align-items: center; justify-content: space-between; font-weight: bold; color: #333; cursor: pointer;">
                <span>📊 Analytics (Statistiche)</span>
                <input type="checkbox" id="chk-analytics" style="transform: scale(1.3);">
            </label>
            <p style="margin: 5px 0 0 0; font-size: 0.8rem; color: #777; font-weight: normal;">Permettono di analizzare il traffico sul portale per migliorarlo.</p>
        </div>

        <div style="margin-bottom: 25px; padding: 12px; border: 1px solid #eee; border-radius: 8px;">
            <label style="display: flex; align-items: center; justify-content: space-between; font-weight: bold; color: #333; cursor: pointer;">
                <span>🎯 AdSense (Pubblicità)</span>
                <input type="checkbox" id="chk-adsense" style="transform: scale(1.3);">
            </label>
            <p style="margin: 5px 0 0 0; font-size: 0.8rem; color: #777; font-weight: normal;">Permettono di mostrare annunci pertinenti per sostenere il sito.</p>
        </div>

        <div style="display: flex; justify-content: flex-end; gap: 10px;">
            <button id="btn-chiudi-modal" style="padding: 10px 20px; border: 1px solid #ccc; background: #fff; border-radius: 6px; cursor: pointer; color: #333; font-weight: bold;">Annulla</button>
            <button id="btn-salva-preferenze" style="padding: 10px 20px; border: none; background: var(--primary, #007bff); color: #fff; border-radius: 6px; cursor: pointer; font-weight: bold;">Salva Preferenze</button>
        </div>
    `;
    ...
    document.getElementById('btn-chiudi-modal').onclick = function() {
        document.body.removeChild(modalOverlay);
    };

    // Azione bottone "Salva Preferenze"
    document.getElementById('btn-salva-preferenze').onclick = function() {
        const valAnalytics = document.getElementById('chk-analytics').checked;
        const valAdsense = document.getElementById('chk-adsense').checked;
        
        salvaPreferenzeGranulari(valAnalytics, valAdsense);
        document.body.removeChild(modalOverlay);
    };
}

function salvaPreferenzeGranulari(analytics, adsense) {
    const banner = document.getElementById('cookie-banner');
    if (banner) banner.style.display = 'none';

    // Memorizza che l'utente ha fatto una scelta granulare e salva gli stati
    localStorage.setItem('consenso_cookie', 'personalizzato');
    localStorage.setItem('consenso_analytics', analytics ? 'granted' : 'denied');
    localStorage.setItem('consenso_adsense', adsense ? 'granted' : 'denied');

    // Aggiorna Google Tag Manager / AdSense con i nuovi valori
    if (typeof gtag === 'function') {
        gtag('consent', 'update', {
            'analytics_storage': analytics ? 'granted' : 'denied',
            'ad_storage': adsense ? 'granted' : 'denied',
            'personalization_storage': adsense ? 'granted' : 'denied',
            'ad_user_data': adsense ? 'granted' : 'denied',
            'ad_personalization': adsense ? 'granted' : 'denied'
        });
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- CONTROLLO COOKIE AL CARICAMENTO ---
    const consenso = localStorage.getItem('consenso_cookie');
    
    if (consenso) {
        // Se c'è già una scelta registrata, nascondiamo il banner
        const banner = document.getElementById('cookie-banner');
        if (banner) {
            banner.style.display = 'none';
        }
        
        // Se la scelta era "accettati", comunichiamo il via libera a Google
        if (consenso === 'accettati') {
            if (typeof gtag === 'function') {
                gtag('consent', 'update', {
                    'analytics_storage': 'granted',
                    'ad_storage': 'granted',
        ...
        } else if (consenso === 'personalizzato') {
            // Se la scelta è stata personalizzata, ricarica i valori salvati nel pannello opzioni
            const valAnalytics = localStorage.getItem('consenso_analytics') || 'denied';
            const valAdsense = localStorage.getItem('consenso_adsense') || 'denied';
            
            if (typeof gtag === 'function') {
                gtag('consent', 'update', {
                    'analytics_storage': valAnalytics,
                    'ad_storage': valAdsense,
                    'personalization_storage': valAdsense,
                    'ad_user_data': valAdsense,
                    'ad_personalization': valAdsense
                });
            }
        }
    }
